...
//			scatterDownscale	= ScatterDownscaleMode.x2
		};
...
//				mr.useLightProbes = false; // this api removed in Unity 5.4
			...
			if(occlusionShader == null)
				occlusionShader = Shader.Find("Hidden/uSkyPro/AtmosphericScattering_Occlusion");

			m_occlusionMaterial = new Material(occlusionShader);
			m_occlusionMaterial.hideFlags = HideFlags.HideAndDontSave;

			m_WhiteTexture = Texture2D.whiteTexture; // built-in 4x4 pixel texture
...
//			FogMultiplier = Mathf.Max (0f, FogMultiplier);
		...
		void OnWillRenderObject() {
			...
			if (scatterOcclusion.useOcclusion) {
				...
				* In this case the Occlusion Texture will be null and get incorrect output, so we assign a white texture (4x4 pixels) here.
				* Whenever m_occlusionCmdBeforeScreen get executed, "occlusionId" will be assigned automatically with Occlusion Texture again.
				*/
				Shader.SetGlobalTexture (occlusionId, m_WhiteTexture );
...
// Properties accessor --------------------
		...
		public bool EnableScattering {
			get { return worldInscatter.enableScattering; }
			set { worldInscatter.enableScattering = value;}
		}
...
//			get { return worldInscatter.scatterDownscale; }
//			set { worldInscatter.scatterDownscale = value;}
//		}

	// Occlusion ---------------------------------

		// set the UseScatterOcclusion
		public bool UseOcclusion {
			get { return scatterOcclusion.useOcclusion;}
			set { scatterOcclusion.useOcclusion = value;}
		}

/*		// Removed this method, and use UseOcclusion above instead
...
*/
		...
		public bool OcclusionDebug {
			get { return scatterOcclusion.occlusionDebug; }
			set { scatterOcclusion.occlusionDebug = value;}
		}
		#endregion
	}
}