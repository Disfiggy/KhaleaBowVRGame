...
//-------------------------------------------------------------------------------------------------	
	...
	#if UNITY_UV_STARTS_AT_TOP
		#if UNITY_VERSION >= 540
			o.uv1 = UnityStereoScreenSpaceUVAdjust(v.texcoord.xy, _MainTex_ST);
		#else
			o.uv1 = v.texcoord.xy;
		#endif
	...
	// Setup a mask based on where the blur origin is
		float  BlurOriginDistanceMask =  saturate(length(_TextureSpaceBlurOrigin.xy - InUV) *InvAspectRatio * 0.5) ; // 0.5 : higher value means faster fade out
...
//-------------------------------------------------------------------------------------------------	
	...
	// Setup a mask based on where the blur origin is
		float  BlurOriginDistanceMask =  saturate(length(_TextureSpaceBlurOrigin.xy - InUV) * 0.5) ;
	// Fade out occlusion over distance away from the blur origin
...
//		float2 uv7 : TEXCOORD7;
	...
	v2f_radial vert_radial( appdata_img v ) {
		...
		o.uv0.xy	=  v.texcoord.xy;// + 0.0 * SampleUVsDelta ;
		o.uv1.xy	=  SampleUVs + 1.0 * SampleUVsDelta ;
		o.uv2.xy 	=  SampleUVs + 2.0 * SampleUVsDelta ;
		o.uv3.xy 	=  SampleUVs + 3.0 * SampleUVsDelta ;
...
//		float InvOcclusionDepthRange = _LightShaftParameters.x; // predefined
		...
		float BlurOriginDistanceMask = 1.0 - saturate(length((_TextureSpaceBlurOrigin.xy - InUV) * InvAspectRatio) * _TextureSpaceBlurOrigin.w);
